...
# Define the endpoint to fetch collections
...
def streamlit_ui():
    ...
    if documents:
        ...
        selected_collection = st.selectbox(
            "Select a collection", list(collections.keys()))
    else:
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader(
            "Choose files", accept_multiple_files=True, type=['txt', 'pdf', 'md'])

        # Ingest files on button click
        if st.button("Ingest Files"):
            if uploaded_files and storage_index:
                files = [('files', file) for file in uploaded_files]
                response = requests.post(INGEST_API, files=files, data={
                                         'index_name': storage_index})
                if response.status_code == 200:
                ...
                st.warning(
                    "Please select a collection and upload files before ingesting.")
    ...
    if user_input and selected_collection:
        ...
        response = requests.post(
            SEARCH_API, data={"index_name": selected_collection, "q": user_input})
